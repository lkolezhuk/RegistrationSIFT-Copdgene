...
%
k=1;%this is the index that corrispond to the number of the image
%
for i=1:size(modality,1)
H=[];
H=computeHomography(Features,Matches(:,:,k), modality{i});

for i=1:size(Features,1)
point_corrispondence{i}=round(H*[Matches(i,2,2),Matches(i,1,2),1]');
intensity1(i)=Im0_gray(round(Features(i,2)),round(Features(i,1)))
point=point_corrispondence{i};
intensity2(i)= Im2_gray(point(1),point(2))
...
H=computeHomography(Features,Matches(:,:,k), 'projective');
%H = inv(H);
